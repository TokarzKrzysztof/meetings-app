...
import { Chip, Icon, IconNames, Stack, Typography } from 'src/ui-components';
      ...
      >
        <Stack gap={1} flexWrap={'wrap'}>
          {userProfile.interests.length ? (
            userProfile.interests.map((x) => (
              <Chip key={x.id} label={x.name} icon={<Icon fontSize={'small'} name={x.iconName as IconNames} />} />
            ))
          ) : (
